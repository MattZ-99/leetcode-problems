...
class Solution {
    void inorderTraversalKernel(TreeNode* root, vector<int> & res) {
        if (!root) return;

        inorderTraversalKernel(root->left, res);
        res.push_back(root->val);
        inorderTraversalKernel(root->right, res);
    }

public:
    vector<int> inorderTraversal(TreeNode* root) {
        vector<int> res;
        inorderTraversalKernel(root, res);
        return res;
    }
};
